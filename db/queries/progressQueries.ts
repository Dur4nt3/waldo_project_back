...
export async function getPlayerCurrentLevel(gameSessionId: number) {
    try {
        const currentLevel = await prisma.gameProgress.findFirst({
            where: {
                gameSessionId,
                finishedAt: null,
            },
...
export async function didUserAlreadyCompleteLevel(
    levelId: number,
    gameSessionId: number,
) {
    try {
        const level = await prisma.gameProgress.findUnique({
            where: {
                unique_individual_level_progress_per_session: {
                    gameSessionId,
                    levelId,
        ...
        return level.finishedAt !== null;
    } catch (error) {
        logError('Error occurred when attempting to check progress', error);
        return null;
    }
}
